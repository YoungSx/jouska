...
## 2024-10-24 - React Component Re-render Bottleneck
**Learning:** `App.tsx` contains heavy IIFE (Immediately Invoked Function Expressions) computations, specifically `dangersByRoute`, calculating object iterations and arrays on every single render. Since `App.tsx` handles top-level routing, modals, and views (meaning it re-renders often), these non-memoized values cause unnecessary recalculations and reference changes, triggering cascading re-renders in children like `RoutesView` when they shouldn't.
**Action:** When finding complex data parsing loops or aggregations within functional components, especially high up in the component tree, proactively wrap them in `React.useMemo` to cache computations based on specific dependencies, preserving reference equality for downstream components and improving CPU load.
