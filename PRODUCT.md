# Product

<!-- impeccable:product-schema 1 -->

## Platform

web

## Users

运维/平台工程师，同时是 jouska 反向代理的部署者与配置者。通常是一到三人的小团队，其中至少一人有 Cloudflare 账号的写权限。

到达面板时的处境有三种，UI 必须分别认得出来：

- **首次部署刚结束**：数据库是空的，没有账号，没有路由。此人刚跑完 `npm run cf:setup`，想知道"接下来干什么"。
- **改一条配置**：面板已在用，此人要改某条路由的上游或超时，然后发布。多数会话属于这一类，且往往在告警之后、时间压力之下发生。
- **事后追查**：某次变更之后代理行为变了，此人要看审计日志找出谁在什么时候改了什么。

## Product Purpose

jouska 是 Hono 上的反向代理中间件，运行在 Cloudflare Workers。管理面板是它的配置平面：把声明式路由表存进 D1，编译成一份文档写入 KV，反代在热路径上读这份 KV。

面板存在的理由是：路由表直接决定生产流量去哪。手改 KV 没有校验、没有审计、没有二次确认，一个错字就能把代理变成内网探测器。面板把这件事变成一个有闸门的流程。

成功的定义是：改一条路由到发布上线，全程不需要打开终端，而且每一次危险的改动都被拦下来问过一次。

## Positioning

面板与反代共用同一份 `jouska` 库的代码来做校验与遮蔽检测。

- 发布前的校验走 `configSchema`（反代运行时用的同一个 zod schema），不是第二套近似实现。
- 遮蔽检测（`shadow.ts`）不重新实现匹配逻辑，而是从每条路由自身的模式生成探测 URL，交给库的 `matchUrl` 判定。所以"这条路由永远收不到流量"这个警告，是由跑在生产上的同一段代码证明的，不会与实际行为漂移。路由条件长到能认请求头、查询参数和 cookie 之后，探测请求也跟着长——探测器会带上条件要求的头、往 URL 里塞参数、拼 Cookie 头，再交同一个 `matchUrl` 判定。

这一点邻近产品无法照抄：它要求配置平面和数据平面共享一个库。

## Operating Context

单 Worker 单源站：面板与它的 API 同源部署（`run_worker_first: ["/api/*"]`），所以 CORS 与 Cookie 不会漂移。

数据流向是单向的：

```
操作者 → 面板 UI → /api/* → D1（草稿：routes / settings / users / audit_log）
                              ↓ 发布（唯一一次 KV 写）
                           CONFIG_KV['routes'] → 反向代理热路径读取
```

关键后果：**D1 里的改动不会立刻生效**。改路由是改草稿；只有"发布"这个动作才影响生产流量。UI 必须一直让这个区别可见，否则操作者会以为保存即上线。

发布是这个 Worker 唯一的 KV 写入，因为免费额度的每日写次数不该被浪费。

角色两种：`admin`（读写、可发布）与 `viewer`（只读）。写操作在各自 handler 上逐个把关，不是按前缀一刀切，所以 viewer 保留全部读取能力。

登录只有一扇门：**Cloudflare Access**。配上 `ACCESS_TEAM` 与 `ACCESS_AUD`，平台在这个 Worker 跑起来之前就把人认完了——面板不存密码、不管会话，以后加登录方式（Cloudflare 自家 IdP、邮件一次性 PIN、GitHub、Google、Okta、通用 OIDC/SAML、硬件密钥）是去 Zero Trust 控制台点几下，不动这个仓库。陌生地址进门发什么号，由部署变量 `ACCESS_PROVISION_ROLE`（在 `wrangler.jsonc` 里，走 PR 评审而非静默 secrets）说了算：设 `admin` 是**等权期**——团队还没有权限故事，谁过门谁 admin；设 `viewer` 是**渐进期**——新人只读，从「用户」页提拔；不设是**创业姿态**——陌生地址被 `403 no_panel_account` 拒之门外，由管理员从「用户」页手工加，因为 Access 策略常常比面板的意图写得宽（整个邮箱域、整个 Cloudflare 账号），谁进门就给谁建号等于把路由表交给一群没人点过名的人。三态之间切换 = 改一行配置 + 重新部署，同一张表、同一批行，零迁移；拼错的值按未设处理（fail closed）并大声记日志。`role` 与 `disabled` 留在 D1，不搬到 Access 分组：最后一个 admin 的守卫只有在写入时原子求值才成立。

唯一盖过政策的两条。其一：`users` 表为空时，第一个过门的人**无条件**成为 admin，变量说什么都没用——清空表是带外恢复手段，恢复不能变成永久锁死。其二：**建号窗口**——部署管线发现面板没有可用的 admin 时（唯一 admin 被删了、被停用了），自动在 D1 的 `settings` 里写一个 7 天期限的窗口行；下一位过门的陌生地址以 admin 落地，且窗口行在**同一批次**里烧掉（一次性）。窗口认的是**状态**不是行：点火前再查一遍「确实没有可用 admin」，过期、损坏、期限远超上限的行一律按关闭处理——一行残留残留不出第二个老板。它存在的意义：管理员被删不该等一次手工 SQL。空表路径仍是它下面的最后一层。

面板自带的那扇密码门已经**永久关闭**：没有 `/api/auth/login`，没有 `sessions` 表，`users` 里没有密码列，也没有带外恢复令牌。换来的不是一张更小的登录表单，而是没有表单——**这份「没有」就是功能本身**。拒绝是终局，请求里带什么都换不来第二个意见；「关掉认证」是操作者在控制台做一次的事，不是攻击者每个请求都能试的事。JWKS 拿不到答 503，不答 401——没有验证材料时没有安全的「是」；没接上 Access 应用的部署是被锁在门外，而不是被悄悄降级成谁都能进。

两个必须明说的后果。

**登出是一次浏览器跳转，不是一次 API 调用**：`POST /api/auth/logout` 什么都不销毁，只回一个同源路径 `/cdn-cgi/access/logout`，跳过去才真的撤销。官方把这个路径挂在应用自己的主机名和 team 域上视为等效——两者都撤销该用户在**所有** Access 应用的会话，已签发令牌 20–30 秒后停止被接受——面板选自己的主机名，因为它会连应用 cookie 一起删掉，退出更即时；而且回一个路径而不是拿 `ACCESS_TEAM` 拼出来的主机名，这个响应就永远不可能变成指向别处的跳转。由此有两件事要知道：后端 `fetch` 一下是退不掉的（撤销发生在边缘，要请求带着用户的 cookie 到），以及 **Access 不支持只退出单个应用**，所以面板上那个「退出登录」会把这个人从组织里所有 Access 应用退出。

**把 `users` 表清空会重新打开首次建号**，下一个被 Access 放行的地址就成了 admin，这正是 `DELETE /api/users/:id` 拒绝删掉最后一行的原因。

**删除用户不是静默的**：删除与吊销名下未过期的 MCP 令牌共享同一批受守卫的写入——令牌**吊销而不删除**（审计留痕），原因记 `owner_deleted`，吊销人记执行删除的 admin，归属链接随 `ON DELETE SET NULL` 消失。确认框先报数量再动手；令牌页把孤儿行置灰成「已撤销 · 主人已删除」，并补出归属列让每把钥匙看得见主人；响应与审计都带 `tokensRevoked`。守卫拒绝时一个字节都不动：没有删了一半的状态。

域名发现是只读查询：面板用一个**只读**的 Cloudflare API token 读出账号里真正能打到反代 Worker 的 hostname（workers.dev 子域、自定义域、zone route），再和路由表交叉比对，回答操作者写 `match.host` 时的两个问题——哪个域名没有路由接管，哪条路由指着一个还没绑定的域名。凭据缺失不是错误：那是受支持的部署，界面解释而不是报警。

## Capabilities and Constraints

已有能力：

- 路由 CRUD、顺序调整（顺序即优先级，首个匹配胜出）、启用/停用。
- 路由级访问控制（`access`）：校验 Cloudflare Access 的 JWT 或 API key 哈希，身份不过关的请求被拒且不转发上游。立场是平台优先——能整域挂 Cloudflare Access 就挂在那儿，路由级只做兜底；面板只负责把哈希而不是明文 key 收进配置。
- 表级 `defaults` 块，逐字段填补空缺。
- 发布预览：编译 + 校验 + 遮蔽检测 + 整站镜像提示 + 危险开关分类，不写 KV。
- 发布：三重闸门（必须编译通过、危险开关需显式 `confirm`、全部进审计日志），一次发布恰好一次 KV 写，`meta` 带 revision/操作者/备注。
- 发布历史与回滚：每次发布存一份快照（含 `defaults` 与全部启用路由，滚动保留 50 版），时间轴可按 revision 浏览、任选两版做服务端字段级 diff；回滚把目标快照恢复进草稿并复用同一条发布管道——它是发布成新 revision，不是倒带计数，`rollbackOf` 记录出处；目标快照先过 schema 校验、再重过全部发布闸门。历史功能之前的旧发布以审计日志回填为「无快照」条目，能看、不能比。
- 舍弃未发布的草稿：把草稿重置为线上正在服务的 revision 的快照。不写 KV、不产生 revision、不过发布闸门——撤销草稿改动不需要为此制造一次发布。快照没有的路由停用但保留定义；快照刻意不做 schema 校验，草稿编译不过（blocked）时照样能弃，这是逃生舱。从未发布过、线上快照不可用、草稿已与线上一致三种情况会拒绝。
- 审计日志，最多 200 条每页。
- 面板登录交给 Cloudflare Access：认证发生在代码之前，免费额度的 10 ms CPU 一分不花，面板不存任何凭据。
- 登出回一个同源路径给前端，跳过去由边缘撤销；范围是该用户的所有 Access 应用，Access 不支持只退一个。
- 第一个过门的人自动成为初始 admin（仅在 `users` 表为空时，且盖过任何政策变量）；之后的建号按 `ACCESS_PROVISION_ROLE` 政策走（等权期全 admin / 渐进期 viewer / 不设则拒），或由管理员在「用户」页手工加。部署时无可启用 admin 会自动开 7 天建号窗口（一次性、认状态不认行、失败仅警告不阻断），作为空表路径之上的恢复层。
- 域名发现：只读列出账号里绑定到反代的 hostname，与路由表交叉比对；60 秒 isolate 内缓存，不落盘。
- 路由级访问控制：委托鉴权（forwardAuth，nginx auth_request 语义）——把「你是谁」交给一个鉴权端点，2xx 放行、其他状态原样回传，端点不可达默认 fail-closed。

技术约束：

- 部署目标是 Cloudflare Workers 的静态资源 + 单 Worker，产物必须是纯静态文件。
- `assets.not_found_handling: "single-page-application"`，所以客户端路由可用。
- 路由定义单条上限 64 KiB，`defaults` 同；发布备注 500 字符；账号名 1–128 字符。
- 存量数据可能损坏：D1 里的 JSON 列若解析失败，读路径必须降级为"具名的问题"，不能 500——否则操作者被锁在他唯一能修好它的那个屏幕外面。

明确的缺口（真实存在的表与字段，但零 UI，未来工作要接上，不许假装已有）：

- 路由级运行时可观测（请求量、错误率、p95、上游健康）的数据源尚不存在，需要 Analytics Engine 之类的接入。

（曾列于此处的「`users` 表没有任何管理界面」已过时：「用户」页已上线，可建号、改角色、停用、删行；唯一剩下的缺口是还没有邮箱邀请流程，加人要手填地址。）

## Brand Commitments

- 产品名 `jouska`，全小写。
- 界面语言中文（zh-CN）。文案集中在一个 messages 模块，当前只有 zh-CN，未来加语言不改组件。
- 语气：说清后果，不吓人。空状态是"还没开始"，不是"配置有错"——刚部署完就说配置有问题只会让人以为装坏了。危险开关不禁止（每一个都有正当用途），只让手指变重。

## Evidence on Hand

真实存在、可直接引用：

- `README.md`：完整的路由选项表（20+ 字段及默认值）、guards、body rewrite 文档。
- `packages/jouska/src/config.ts`：权威 zod schema，含归一化规则与安全注释。
- `workers/admin-panel/src/danger.ts`：9 条危险字段分类，每条带 level 与人类可读的 reason。
- `workers/admin-panel/src/shadow.ts`：遮蔽检测，产出 `shadowedId` / `byId` / `probe`。
- `workers/admin-panel/src/mirror.ts`：整站镜像提示，产出 `routeId` / `upstream`；判据只覆盖整站路由，且是提示不是错误。
- `workers/admin-panel/migrations/`：`0001_init.sql`、`0002_tables.sql`、`0003_revisions.sql` 的真实表结构。
- `workers/admin-panel/src/cloudflare.ts`：域名发现的三个来源与它们各自的失败模式。

不存在，不许编造：性能基准数字、用户数量、客户案例、可用性承诺、任何路由的实时流量数据。

## Product Principles

1. **草稿与生产是两件事，UI 永远让这个区别可见。** 保存不等于上线；发布是唯一改变生产流量的动作。
2. **危险不禁止，只让手指变重。** 每个危险开关都有正当用途，面板的职责是让操作者在按下去之前确实看见后果。
3. **校验用生产的那份代码，不用第二个意见。** 任何"这样不对"的判断都必须由反代实际运行的同一段逻辑得出，否则警告会与真实行为漂移。
4. **空不是坏。** 空路由表、空审计、首次部署都是正常状态，要引导下一步，不要报错。
5. **损坏的数据必须能在界面里被看见并修好。** 读路径遇到坏数据降级为具名问题，绝不把操作者挡在修复界面之外。

## Accessibility & Inclusion

面板可能在告警中、深夜、单手操作下使用。键盘可达与焦点可见是硬要求，不是加分项：全部交互控件必须可 Tab 到达、焦点环可见、Dialog 有焦点陷阱与 Esc 关闭。这也是选用 Base UI 的直接理由。
